    ...
    if [[ -v SHS_NEW_BUILD_SYSTEM ]]; then
      case "${TARGET_OS}" in
        rhel*)
          ...
          ;;
        sle*)
          kernels="$(ls /usr/src/linux-obj/${TARGET_ARCH})"
          ;;
      esac
    else
...
for kver in $kernels; do
    ...
    echo "BUILD_METADATA='0' rpmbuild -ba ./WORKSPACE/SPECS/$SPECFILE_BASENAME --define \"_topdir $TOPDIR\" --define \"kernel_version $kver\" --target \"$ARCH\" --define \"_os $OS\" --define \"_arch $ARCH\" --define \"_buildname $BUILDNAME\" --define \"debug_package $DEBUG_PACKAGE\" " > WORKSPACE/helper.sh
