...
#undef MODULE_NAME
#endif
#define MODULE_NAME "test_sep_cq_rx"

#define MAX_RX_CNT 10
#define MAX_RX_CNT 10

MODULE_AUTHOR("Ian Ziemba");
MODULE_DESCRIPTION("kfabric KCXI SEP CQ RX CRX tests");
...
static int test_rx_ctx_single_bind_cq(int id)
{
	...
	if (rc) {
		...
		LOG_ERR("Failed to open RX CRX");
		return rc;
	}
	rx_ctx = container_of(rx_ep, struct kcxi_rx_ctx, ctx);
	...
	if (rx_ctx->suppress_events) {
		LOG_ERR("TEST %d %s FAILED", id, __func__);
		LOG_ERR("RX contexts selective comp set");
		exit_rc = -EINVAL;
		goto out;
	}
...
static int test_rx_ctx_single_bind_cq_selective_comp(int id)
{
	struct kfid_ep *rx_ep;
	struct kcxi_rx_ctx *rx_ctx;
	int exit_rc = 0;
	...
	rc = kfi_ep_bind(rx_ep, &cq->fid, KFI_SELECTIVE_COMPLETION);
	if (rc) {
		LOG_ERR("TEST %d %s FAILED", id, __func__);
		LOG_ERR("Failed to bind CQ");
		exit_rc = rc;
	...
	if (!rx_ctx->suppress_events) {
		LOG_ERR("TEST %d %s FAILED", id, __func__);
		LOG_ERR("RX contexts selective comp not set");
		exit_rc = -EINVAL;
		goto out;
	}
...
static int test_rx_ctx_double_bind_cq(int id)
{
	struct kfid_ep *rx_ep;
	int exit_rc = 0;
	int rc;
...
static int test_rx_ctx_premature_cq_close(int id)
{
	struct kfid_ep *rx_ep;
	int exit_rc = 0;
	int rc;
...
static int __init test_module_init(void)
{
	...
	rc = test_rx_ctx_single_bind_cq(test_id);
	if (rc)
		exit_rc = rc;
	test_id++;

	rc = test_rx_ctx_single_bind_cq_selective_comp(test_id);
	if (rc)
		exit_rc = rc;
	test_id++;

	rc = test_rx_ctx_double_bind_cq(test_id);
	if (rc)
		exit_rc = rc;
	test_id++;

	rc = test_rx_ctx_premature_cq_close(test_id);
	if (rc)
		exit_rc = rc;
	test_id++;
