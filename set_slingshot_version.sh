...
echo "INFO: Branch: $BRANCH_NAME Release: $PRODUCT_VERSION Label: $PRODUCT_LABEL"
sed -i -e "s/%define release_extra.*/%define release_extra ${PRODUCT_LABEL}${PRODUCT_VERSION}/g" cray-kfabric.spec
sed -i -e "s/Release:.*/Release: ${PRODUCT_LABEL}${PRODUCT_VERSION}_%(echo \\\${BUILD_METADATA:-1})/g" cray-kfabric.spec
